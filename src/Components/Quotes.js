...
const Quotes = ({description, image, isReverse}) => {
  return (
    <div
      ...
      } items-center gap-10 px-4 sm:px-6 shadow-2xl relative overflow-hidden`}
    >
      <div className="inset-0 z-20 sm:w-2/3 xxs:max-sm:relative">
        <pre className="text-xl text-gray-200 whitespace-pre-line sm:text-justify font-Titillium xxs:max-sm:py-8 ">
          {description}
        </pre>
      </div>
      ...
      <div className="relative inset-0 z-10 overflow-hidden sm:w-1/3 rounded-2xl xxs:max-sm:absolute">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-60 sm:hidden"></div>
      </div>
    </div>
  );
}
