...
const TechCard = ({ title, description, image, reverse, id }) => {
  ...
  return (
    <div
      className={`flex ${
        reverse ? `flex-row-reverse` : `flex-row pl-4`
      } max-w-6xl w-full at460:w-11/12  shadow-xl py-8 gap-x-8  relative xxs:max-at602:h-[60vh] at602:max-at830:h-[55vh]`}
    >
      <div className="w-full h-full space-y-7 sm:space-y-8 xxs:max-at830:absolute xxs:max-at830:inset-0 xxs:max-at830:z-20 xxs:max-at830:flex xxs:max-at830:flex-col xxs:max-at830:items-center at830:w-2/3 xxs:max-sm:pt-8 xxs:max-sm:px-4 sm:max-at830:p-8">
        <h1 className="text-4xl font-medium text-gray-100 font-Titillium xxs:max-at830:text-center">
          {title}
        </h1>
        <p className="text-2xl font-medium text-gray-300 at830:text-xl xxs:max-at830:text-center font-Titillium">
          {description}
        </p>
        <button
          onClick={handleClick}
